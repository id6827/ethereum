App = {
  ...
  loadRealEstates: function() {
    App.contracts.RealEstate.deployed().then(function(instance) {
      ...
      for (i = 0; i < buyers.length; i++) {
        if (buyers[i] !== '0x0000000000000000000000000000000000000000') { // 0이 40개 (초기화)
  ...
  listenToEvents: function() {
    App.contracts.RealEstate.deployed().then(function(instance) {
      instance.LogBuyRealEstate({filter:{}, fromBlock:0, toBlock:'latest'}, function(error, event) {
        if (!error) {
...
$(function() {
  ...
  $('#buyerInfoModal').on('show.bs.modal', function(e) {
    ...
    App.contracts.RealEstate.deployed().then(function(instance) {
      ...
      var nameUtf8Decoded = utf8.decode(buyerInfo[1]);
