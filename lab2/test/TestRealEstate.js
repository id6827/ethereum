...
contract('RealEstate', function(accounts) {
    ...
    it("가나슈 두번째 계정으로 매물 아이디 0번 매입 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스팅", function() {
        return RealEstate.deployed().then(function(instance) { 
            realEstateInstance = instance;
            return realEstateInstance.buyRealEstate(0, "dowon", 13, {from: accounts[1], value: web3.utils.toWei("1.50", "ether")});
            ...
            //assert.equal(web3.toAscii(data[1]).replace(/\0/g, ''), "dowon", "매입자의 이름이 dowon이 아닙니다.");
            assert.equal(buyerInfo[1], "dowon", "매입자의 이름이 dowon이 아닙니다.");
            assert.equal(buyerInfo[2], 13, "매입자의 나이가 13살이 아닙니다.");
            return realEstateInstance.getAllBuyers();
        }).then(function(buyers) {
            assert.equal(buyers[0].toUpperCase(), accounts[1].toUpperCase(), "Buyers배열 첫번째 인덱스의 계정이 가나슈 두번째 계정과 일치하지 않습니다.");
        }); 
    });
})